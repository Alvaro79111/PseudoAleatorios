"""Interfaz para el generador de Cuadrados Medios."""
import tkinter as tk
from tkinter import ttk, messagebox
from ..generators import cuadrados_medios
from .. import tests
import pandas as pd
import datetime
...
class CuadradosMediosApp:
    ...
    def setup_parametros(self):
        for widget in self.frame.winfo_children():
            widget.destroy()
        ttk.Label(self.frame, text="Cuadrados Medios", font=("Arial", 18, "bold"), foreground="#2563eb").pack(pady=(18, 8))
    ...
    def calcular(self):
        try:
            ...
            return
        self.resultados = cuadrados_medios(seed, n)
    ...
    def mostrar_tabla(self, resultados, valores):
        for widget in self.frame.winfo_children():
            widget.destroy()
        # --- Tabla de resultados ---
        tabla_frame = ttk.LabelFrame(self.frame, text="Resultados - Cuadrados Medios", style="Custom.TLabelframe")
        tabla_frame.pack(fill="x", padx=10, pady=10)
        cols = ["Xi", "Xi^2", "Medio", "Ri"]
        ...
        def exportar_excel():
            import pandas as pd
            import datetime
            df = pd.DataFrame(resultados, columns=["Xi", "Xi^2", "Medio", "Ri"])
            nombre = f"cuadrados_medios_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
    ...
    def mostrar_pruebas(self, valores):
        ...
        ttk.Label(self.frame, text="Pruebas estadísticas", font=("Arial", 16, "bold"), foreground="#2563eb").pack(pady=(8, 8))
...
"""
        ...
        ttk.Label(frame_medias, text="Prueba de Medias", font=("Arial", 13, "bold"), foreground="#2563eb").pack(pady=6)
...
"""
        ...
        ttk.Label(frame_var, text="Prueba de Varianza", font=("Arial", 13, "bold"), foreground="#2563eb").pack(pady=6)
...
"""
        ...
        ttk.Label(frame_uni, text="Prueba de Uniformidad", font=("Arial", 13, "bold"), foreground="#2563eb").pack(pady=6)

"""Interfaz para el generador de Productos Medios."""
import tkinter as tk
from tkinter import ttk, messagebox
from ..generators import productos_medios
from .. import tests
...
class ProductosMediosApp:
    ...
    def setup_parametros(self):
        for widget in self.frame.winfo_children():
            widget.destroy()
        ttk.Label(self.frame, text="Productos Medios", font=("Arial", 18, "bold"), foreground="#059669").pack(pady=(18, 8))
        ...
        self.n_entry.focus()
        ttk.Label(self.frame, text="Semilla X (mínimo 4 dígitos):", font=("Arial", 13)).pack(pady=6)
        self.seedx_entry = ttk.Entry(self.frame, font=("Arial", 13), width=12)
        self.seedx_entry.pack(pady=6)
        ttk.Label(self.frame, text="Semilla Y (mínimo 4 dígitos):", font=("Arial", 13)).pack(pady=6)
        self.seedy_entry = ttk.Entry(self.frame, font=("Arial", 13), width=12)
        self.seedy_entry.pack(pady=6)
    ...
    def calcular(self):
        try:
            ...
            if n <= 0 or len(str(seedx)) < 4 or len(str(seedy)) < 4:
                raise ValueError
        except:
            messagebox.showerror("Error", "Entradas inválidas")
            return
        resultados = productos_medios(seedx, seedy, n)
        valores = [r[4] for r in resultados]
        self.mostrar_tabla(resultados, valores)

    def mostrar_tabla(self, resultados, valores):
        for widget in self.frame.winfo_children():
            widget.destroy()
        # --- Tabla de resultados ---
        tabla_frame = ttk.LabelFrame(self.frame, text="Resultados - Productos Medios", style="Custom.TLabelframe")
        tabla_frame.pack(fill="x", padx=10, pady=10)
        cols = ["Xi", "Yi", "Xi*Yi", "Medio", "Ri"]
        ...
        def exportar_excel():
            import pandas as pd
            import datetime
            df = pd.DataFrame(resultados, columns=["Xi", "Yi", "Xi*Yi", "Medio", "Ri"])
            nombre = f"productos_medios_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
    ...
    def mostrar_pruebas(self, valores):
        for widget in self.frame.winfo_children():
            widget.destroy()
        ttk.Label(self.frame, text="Pruebas estadísticas", font=("Arial", 16, "bold"), foreground="#059669").pack(pady=(18, 8))
        ...
        ttk.Button(self.frame, text="Ejecutar pruebas", command=ejecutar, style="Custom.TButton").pack(pady=12)
...
"""
        ...
        ttk.Label(frame_medias, text="Prueba de Medias", font=("Arial", 13, "bold"), foreground="#059669").pack(pady=6)
...
"""
        ...
        frame_var.grid(row=1, column=0, sticky="nsew", padx=(0, 8))
        ttk.Label(frame_var, text="Prueba de Varianza", font=("Arial", 13, "bold"), foreground="#059669").pack(pady=6)
...
"""
        ...
        frame_uni.grid(row=2, column=0, sticky="nsew", padx=(0, 8))
        ttk.Label(frame_uni, text="Prueba de Uniformidad", font=("Arial", 13, "bold"), foreground="#059669").pack(pady=6)
        ttk.Label(frame_uni, text=txt_uni, justify="left", font=("Arial", 12)).pack(padx=4, pady=4)
        # Tabla de uniformidad a la derecha de las columnas
        tabla_uni_frame = ttk.Frame(main_content)
        tabla_uni_frame.grid(row=0, column=1, rowspan=3, sticky="nsew", padx=(8, 0))
        ...
        # Agregar scrollbar a la tabla de uniformidad si es necesario
        ...
        tabla_uni_frame.bind("<Configure>", lambda e: on_frame_configure(self.canvas))
        ttk.Button(self.frame, text="Volver", command=self.volver_callback, style="Custom.TButton").pack(pady=8)
